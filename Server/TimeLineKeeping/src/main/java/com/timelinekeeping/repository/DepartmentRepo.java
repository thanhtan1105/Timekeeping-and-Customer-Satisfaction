...
public interface DepartmentRepo extends JpaRepository<DepartmentEntity, Long> {
    ...
    @Query("SELECT d.accountEntitySet FROM DepartmentEntity d WHERE d.id = ?1 AND d.active <> 0")
    Page<AccountEntity> findByDepartment(Long departmentId, Pageable pageable);
